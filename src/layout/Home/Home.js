...
const Home = () => {
  ...
  useEffect(() => {
    ...
    eventService.getAllPublicEvents()
      .then((res) => {
        if (__isMounted) {
          ...
          setPublicEventMarkers({ markers, loaded: true });
        }
      }).catch(() => {
        if (__isMounted) {
    ...
    eventService.getAllFriendsEvents()
      .then((res) => {
        if (__isMounted) {
        ...
        <div className="map-cont">
          {
            ...
            <MapBox
              zoom={5}
              showPopups
              latitude={publicEventMarkers.markers[0].lat}
              longitude={publicEventMarkers.markers[0].long}
